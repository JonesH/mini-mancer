...
class AdaptiveRateLimiter:
    ...
    def _adjust_rate_after_429(self, bot_token: str, retry_after: int | None = None) -> None:
        ...
        consecutive_429s: int = bucket["consecutive_429s"]
        reduction_factor = 0.5 ** consecutive_429s
        bucket["current_rate"] = max(self.min_rate, int(self.base_rate * reduction_factor))

        # Set backoff period based on Retry-After header or exponential backoff
        if retry_after:
            bucket["backoff_until"] = time.time() + retry_after
            logger.warning(
                f"Bot {bot_token[:10]}: 429 error, backing off for {retry_after}s (Retry-After header)"
            )
        else:
            backoff_time = min(60, 2 ** consecutive_429s)  # Max 60s backoff
            bucket["backoff_until"] = time.time() + backoff_time
            logger.warning(
                f"Bot {bot_token[:10]}: 429 error #{bucket['consecutive_429s']}, backing off for {backoff_time}s"
            )

        logger.info(
            f"Bot {bot_token[:10]}: Rate reduced from {self.base_rate} to {bucket['current_rate']}/sec"
        )

    def _maybe_recover_rate(self, bot_token: str) -> None:
    ...
    async def wait_if_needed(self, bot_token: str) -> None:
        ...
        if bucket["tokens"] < 1:
            ...
            logger.debug(
                f"Bot {bot_token[:10]}: Rate limiting at {current_rate}/sec, waiting {wait_time:.2f}s"
            )
            await asyncio.sleep(wait_time)
            bucket["tokens"] = 0
        else:
    ...
    def handle_successful_call(self, bot_token: str) -> None:
...
async def rate_limited_call(bot_token: str, coro: Any) -> Any:
    ...
    try:
        ...
        try:
            ...
            await log_api_call(
                f"{method_name}_429_retry", bot_token, error="RetryAfter", retry_after=e.retry_after
            )
        except ImportError:
            pass
        ...
        if "429" in str(e) or "Too Many Requests" in str(e):
            ...
            try:
                ...
                await log_api_call(
                    f"{method_name}_429_error", bot_token, error=str(e), retry_after=retry_after
                )
            except ImportError:
                pass
